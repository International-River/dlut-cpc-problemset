...
## 现状：M0（规范）+ M2（构建）+ M3（前端）+ M4（投稿）+ M8（部署）已就绪
...
- **M8 CI/CD 与部署**——push 到 `main` 自动构建并部署到 GitHub Pages。
...
npm run fmt          # 统一所有 problems/*/meta.yml 的 YAML 风格
npm run fmt:check    # 只检查格式是否规范（CI 用，未格式化则报错）
```
...
三条工作流各司其职（Node 20 统一）：

- **`validate.yml`**——PR / push 到 `main` 时跑 `npm ci && npm run fmt:check && npm run check`，做数据红绿灯。
- **`intake.yml`**——投稿 Issue 打上 `approved-intake` 标签后转成 PR。
- **`deploy.yml`**（本模块）——push 到 `main`（或手动 `workflow_dispatch`）时，**按固定顺序**构建后发布到 GitHub Pages：
  1. 根目录 `npm ci && npm run build` → 生成 `dist/data.json`；
  2. `cd web && npm ci && npm run build` → 其 `prebuild` 把上一步的 `data.json` 复制进来，产出 `web/dist/`；
  3. 官方 Pages 动作（`configure-pages` / `upload-pages-artifact` / `deploy-pages`）发布 `web/dist/`。

  顺序不能颠倒：先有根产物 `data.json`，前端才有数据。两处 `npm ci` 均开启 npm 缓存；`permissions` 收敛为 `pages: write` + `id-token: write` + `contents: read`，`concurrency` 组 `pages` 保证同一时刻只发布一份。

站点为 **project pages**，地址 `https://international-river.github.io/dlut-cpc-problemset/`。前端 `base: './'` + hash 路由（`#/p/...`），资源与 `data.json` 均以相对路径加载，可在该子路径下正确取到。

> **一次性人工设置（必须）**：仓库 **Settings → Pages → Source 选 “GitHub Actions”**。
> 选定后，向 `main` 推送即会自动触发部署；也可在 **Actions → 部署 Pages → Run workflow** 手动触发。

> 月度**死链检查**（SPEC §8.8 的软校验）属 M7 兜底范畴，需配套一个链接探活脚本，尚未实现；
> 待 M7 落地后再补一个 `schedule` 触发、**不阻断部署**的软校验工作流。
...
scripts/fmt.mjs               # meta.yml 格式化 / 格式检查（统一 YAML 风格）
...
.github/workflows/            # validate.yml（校验）、intake.yml（投稿转 PR）、deploy.yml（部署 Pages）
